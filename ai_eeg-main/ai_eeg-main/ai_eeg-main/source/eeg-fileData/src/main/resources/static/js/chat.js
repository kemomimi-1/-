...
// ========== 流式发送消息（SSE 实时输出）==========
    async function sendMessage() {
        ...
        try {
            ...
            if (response.status === 401) {
                removeStreamingMessage(streamMsg.messageDiv);
                handleUnauthorized();
                return;
            }
            ...
            if (!response.ok) {
                removeStreamingMessage(streamMsg.messageDiv);
                addMessageToChat('assistant', '抱歉，服务器返回错误 (HTTP ' + response.status + ')');
                return;
            }
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            switch (data.type) {
                                case 'thinking':
                                    // 显示思考/工具调用状态
                                    const thinkingTextEl = streamMsg.thinkingEl.querySelector('.thinking-text');
                                    if (thinkingTextEl) {
                                        thinkingTextEl.textContent = data.content || '🤔 正在思考';
                                    }
                                    streamMsg.thinkingEl.style.display = 'flex';
                                    break;

                                case 'content':
                                    // 隐藏思考状态，追加内容并实时渲染
                                    streamMsg.thinkingEl.style.display = 'none';
                                    fullContent += data.content;
                                    if (fullContent.trim()) {
                                        streamMsg.bubbleEl.style.display = 'block';
                                        streamMsg.bubbleEl.innerHTML = formatAIResponse(fullContent) +
                                            '<span class="typing-cursor"></span>';
                                    }
                                    // 自动滚动到底部
                                    const chatMsgs = DOM_CACHE.chatMessages;
                                    chatMsgs.scrollTo({ top: chatMsgs.scrollHeight, behavior: 'smooth' });
                                    break;

                                case 'session':
                                    // 接收会话ID
                                    if (data.content && !currentConversationSessionId) {
                                        currentConversationSessionId = data.content;
                                        DOM_CACHE.chatTitle.textContent =
                                            '对话 - ' + data.content.substring(0, 8);
                                    }
                                    break;

                                case 'tools':
                                    // 接收工具使用信息
                                    try { toolsUsed = JSON.parse(data.content); } catch(e) {}
                                    break;

                                case 'done':
                                    // 移除光标，显示最终格式化内容
                                    streamMsg.bubbleEl.innerHTML = formatAIResponse(fullContent);
                                    streamMsg.bubbleEl.classList.remove('streaming');
                                    break;
                                ...
                                case 'error':
                                    ...
                                    streamMsg.thinkingEl.style.display = 'none';
                                    streamMsg.bubbleEl.style.display = 'block';
                                    streamMsg.bubbleEl.innerHTML = '<span style="color: #ef4444;">❌ ' + data.content + '</span>';
                                    streamMsg.bubbleEl.classList.remove('streaming');
                                    break;
                            }
                        } catch (e) {
            ...
            streamMsg.thinkingEl.style.display = 'none';
            if (hasError) {
                // 如果后端已经明确推送了错误，保留该错误提示，不要被覆盖
            } else if (fullContent.trim()) {
                streamMsg.bubbleEl.style.display = 'block';
                streamMsg.bubbleEl.innerHTML = formatAIResponse(fullContent);
            } else {
                streamMsg.bubbleEl.style.display = 'block';
                streamMsg.bubbleEl.innerHTML = '<span style="color: #ef4444;">❌ 服务器未返回内容，连接可能异常中断。</span>';
            }
            streamMsg.bubbleEl.classList.remove('streaming');

            // 显示工具使用标签
            updateStreamingMessageMeta(streamMsg.messageDiv, toolsUsed);
...
// ========== 流式消息辅助函数 ==========
    ...
    function createStreamingMessage() {
        ...
        messageDiv.innerHTML = `
            ...
            <div class="message-content">
                <div class="streaming-thinking" style="display:flex;">
                    <span class="thinking-text">🤔 正在思考</span><div class="thinking-dots"><span></span><span></span><span></span></div>
                </div>
                <div class="message-bubble streaming" style="display:none;"><span class="typing-cursor"></span></div>
                <div class="message-meta">
                    <span class="message-time">${time}</span>
                    <div class="tools-used"></div>
                </div>
            </div>
        `;
        ...
        return {
            messageDiv,
            bubbleEl: messageDiv.querySelector('.message-bubble'),
            thinkingEl: messageDiv.querySelector('.streaming-thinking')
        };
    }
    ...
    // 更新流式消息的工具使用标签
    function updateStreamingMessageMeta(messageDiv, toolsUsed) {
        if (!toolsUsed || toolsUsed.length === 0) return;
        const toolsContainer = messageDiv.querySelector('.tools-used');
        if (toolsContainer) {
            toolsContainer.innerHTML = toolsUsed
            ...
            <div class="message assistant">
                ...
                <div class="message-content">
                    <div class="message-bubble">
                        ...
                        • 提供实时脑电状态分析<br>
                        <br>
                        如需开始数据传输，请点击右侧的连接控制面板。如需查看实时频段数据，请切换到"实时监测"页面。有什么我可以帮助您的吗？
                    </div>
                    <div class="message-meta">
