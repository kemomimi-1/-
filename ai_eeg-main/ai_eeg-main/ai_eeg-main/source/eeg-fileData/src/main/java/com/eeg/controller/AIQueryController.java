...
import com.eeg.service.AIStreamService;
...
public class AIQueryController {
    ...
    /**
     * 【新增】流式AI查询处理接口 - SSE实时流式输出
     * 实现逐字/逐句实时返回AI回复，同时展示思考过程
     */
    @PostMapping(value = "/query/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<ServerSentEvent<String>> processUserQueryStream(@RequestBody AIQueryRequest request,
                                                               HttpSession httpSession) {

        Long userId = (Long) httpSession.getAttribute("userId");
        if (userId == null) {
            return Flux.just(createErrorEvent("用户未登录"));
        }

        if (request == null || request.getQuery() == null || request.getQuery().trim().isEmpty()) {
            return Flux.just(createErrorEvent("查询内容不能为空"));
        }

        String userQuery = request.getQuery().trim();
        String sessionId = request.getSessionId();

        log.info("收到流式AI查询请求 - 用户ID: {}, 查询长度: {} 字符", userId, userQuery.length());
        ...
        try {
            ...
            if (sessionId == null || sessionId.trim().isEmpty()) {
                ConversationSession newSession = conversationHistoryService.createNewConversationSession(userId);
                sessionId = newSession.getSessionId();
                log.info("流式查询 - 新会话已创建: {}", sessionId);
            }

            final String finalSessionId = sessionId;

            // 构建智能协作上下文（复用现有逻辑，不修改原方法）
            Map<String, Object> context = buildEnhancedIntelligentContext(userId, request, userQuery);
            context.put("conversationSessionId", finalSessionId);

            // 让AIStreamService处理流并在后台线程中发回Flux
            return aiStreamService.processStreamQuery(userId, userQuery, context, finalSessionId);

        } catch (Exception e) {
            log.error("流式查询预处理失败 - 用户ID: {}", userId, e);
            return Flux.just(createErrorEvent("请求处理失败: " + e.getMessage()));
        }
    }
    ...
    private ServerSentEvent<String> createErrorEvent(String errorMessage) {
        try {
            Map<String, String> errorData = Map.of("type", "error", "content", errorMessage);
