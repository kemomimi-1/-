...
- **定时执行：** 每7秒自动执行一次分析（通过`application.yml`中`eeg.realtime-analysis.analysis-interval-seconds`配置，已从默认30秒调整为7秒）
- **批量处理：** 为所有活跃用户执行分析
...
原有的弹幕内容由固定模板拼接生成（如"【专注状态】 主导: BETA (置信度: 85.2%) - 当前专注度良好"），内容固定、重复度高。本次开发中引入了AI大模型（通义千问 qwen-max），让弹幕内容由AI根据每次分析的具体数值差异生成专家水准的个性化分析报告。

### 6.2 修改的代码文件

**RealTimeSpectrumAnalysisService.java** — 新增 `generateAIExpertAnalysis()` 方法，修改 `createBarrageMessage()` 的调用链，新增 `AIModelService` 依赖注入。

### 6.3 新增方法：`generateAIExpertAnalysis()`

在 `createBarrageMessage()` 中，弹幕内容的生成从原来直接调用 `generateBarrageContent()` 模板方法，改为优先调用 `generateAIExpertAnalysis()`，如果AI调用失败则回退到原有模板逻辑。

```java
// createBarrageMessage() 中的关键变更：
// 原来：String content = generateBarrageContent(result, data);
// 现在：
...
private String generateAIExpertAnalysis(Long userId, AnalysisResult result, SpectralData data) {
    try {
        ...
        prompt.append("任务：\n");
        prompt.append("1. 必须以【").append(result.getPrimaryState().getDescription()).append("】作为回复的最开头。\n");
        prompt.append("2. 结合本次的具体数值比例特征，给出差异化的分析说明。必须引用至少1个最突出的具体数值。\n");
        prompt.append("3. 以专业医学口吻提供一段精炼的脑电特征分析报告 (总字数严格控制在80字以内)。\n");
        prompt.append("4. 结尾附带一句具体的微操作建议。\n");
        prompt.append("5. 不要包含任何HTML标签，只需纯文本。不要使用前奏语，直接进入状态分析。\n");

        // 使用阻塞调用（在 @Async 异步任务中执行，安全）
        AIModelService.AIResponse response = aiModelService.processUserQuery(
            userId, prompt.toString(), new HashMap<>()).block();
    ...
    // --- 回退逻辑 (Fallback): 使用原有的模板拼接 ---
    return generateBarrageContent(result, data);
}
```

- **Prompt设计要点：** 要求AI以`【状态标签】`开头，引用至少1个具体数值，控制在80字以内，避免前奏语和HTML标签
- **回退机制：** 如果AI调用失败（超时、网络异常等），自动使用原有的 `generateBarrageContent()` 模板方法，保证功能不中断
- **新增依赖：** Service构造器中新增了 `AIModelService aiModelService` 参数

### 6.4 原有模板方法（保留为Fallback）
...
## 7. 【开发变更】配置参数调整与连接门控移除

### 7.1 配置参数调整

以下参数在 `application.yml` 中进行了调整，以提供更实时的分析体验：

| 参数 | 原始值 | 调整后 | 修改原因 |
|:---|:---|:---|:---|
| `analysis-interval-seconds` | 30 | 7 | 缩短分析间隔，提供更实时的反馈 |
| `sample-window-minutes` | 2 | 1 | 缩小采样窗口，分析更近期的数据 |
| `min-samples` | 10 | 2 | 降低最低样本要求，避免数据量少时无法触发分析 |

**修改后的 application.yml 配置：**

```yaml
eeg:
  realtime-analysis:
    sample-window-minutes: 1
    min-samples: 2
    analysis-interval-seconds: 7
```

### 7.2 移除连接门控

**变更背景：** 原有实现中，前端的"开始分析"按钮必须在用户有活跃的EEG设备连接时才能点击，这限制了用户对已有历史数据的分析能力。

**变更内容：**
- 前端 `app.js` 中 `startRealTimeAnalysis()` 函数移除了连接状态检查逻辑
- "开始分析"按钮在任何情况下都可以点击
- 系统会自动从InfluxDB中已有的历史数据进行分析
- 如果没有任何数据，分析会因为样本数不足而跳过，不会报错

### 7.3 前端通知方式优化

**变更内容：** 将弹幕相关的弹窗提示（`alert()`）全部替换为 `console.log()` 和 `console.warn()`，避免打断用户操作。

### 7.4 新增接口：`GET /api/realtime-analysis/status`

控制器中新增了分析状态查询接口，用于前端获取当前用户的实时分析是否正在运行：
...
`getContinuousSpectralData()` 方法拆分为 `getContinuousSpectralDataWithDepth(userId, depth)` 带深度计数器的版本，最多重试1次，防止历史数据为空时的无限递归：
...
- **AI回退降级：** AI分析调用失败时自动使用模板方法生成弹幕内容
- **递归深度保护：** 数据采样增加深度计数器，防止无限递归
- **异常隔离：** 单用户分析失败不影响其他用户
...
- **即时反馈：** 启动分析后立即获得结果
- **连续监测：** 7秒间隔的持续分析（已从30秒缩短）
- **AI专家报告：** 弹幕内容为AI生成的个性化专业分析，避免千篇一律
- **无连接限制：** 即使没有活跃EEG连接也可以分析历史数据
- **无弹窗干扰：** 操作反馈通过console日志输出，不弹窗打断用户
- **历史回顾：** 完整的分析历史记录
...
- **依赖服务：** 通过 `AIModelService` 调用通义千问 qwen-max 大模型
- **调用方式：** 在 `@Async` 异步定时任务中使用 `.block()` 阻塞调用
- **Prompt工程：** 设计了包含具体输出格式要求的医学诊断Prompt
- **容错保障：** 调用失败时自动降级到模板生成，不影响弹幕功能
...
弹幕内容由AI大模型根据每次分析的具体频段数值动态生成，以临床神经内科医生的专业口吻输出，每条弹幕都具有针对当前数据的独特性，避免了传统模板方式的重复和单调。
