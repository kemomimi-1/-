...
eeg:
  ...
  realtime-analysis:
    sample-window-minutes: 1            # 已从2分钟缩短为1分钟
    min-samples: 2                      # 已从10降低为2
    analysis-interval-seconds: 7        # 已从30秒缩短为7秒
    enable-auto-start: false            # 已关闭自动启动，改为手动触发
...
- **实时分析**：1分钟采样窗口、7秒分析间隔的实时处理配置（已从原有的2分钟窗口/30秒间隔调整）
...
原有的AI查询接口为同步请求-响应模式（`POST /api/ai/query`），用户提交问题后需要等待AI和所有MCP工具调用完成后才能看到回复。当查询复杂度高或工具调用较多时，用户等待时间较长且无任何反馈。

本次开发中新增了SSE（Server-Sent Events）流式输出功能，AI回复可以逐字逐句实时推送到前端，同时展示工具调用状态等"思考过程"，大幅提升了交互体验。

### 6.2 新增文件

#### `AIStreamService.java`（新增，510行）

独立于 `AIModelService` 的流式输出服务，使用 `HttpURLConnection` 直接发送流式HTTP请求（`stream: true`）到通义千问API，逐行读取SSE响应并实时推送给前端。

**核心方法：**

```java
public Flux<ServerSentEvent<String>> processStreamQuery(
    Long userId, String userQuery, Map<String, Object> context, String sessionId)
```
...
| 事件类型 | 含义 | 示例内容 |
|:---|:---|:---|
| `thinking` | AI思考/工具执行状态 | "🔧 正在执行: queryLatestBandPowerData..." |
| `session` | 会话ID通知 | "conv_xxx_xxx" |
| `content` | AI回复内容增量 | "根据您的EEG数据分析..." |
| `tools` | 使用的工具列表 | ["queryLatestBandPowerData","getSessionDetails"] |
| `done` | 流式输出完成 | "" |
| `error` | 错误信息 | "处理失败: ..." |

**关键实现细节：**

- **流式工具调用处理**：流式响应中 `tool_calls` 是分片到达的，使用 `ToolCallAccumulator` 内部类逐步拼接工具名和参数
- **递归支持**：工具调用后将结果发回AI，AI可继续调用工具或输出文本回复，递归深度受 `maxToolCalls` 限制
- **大数据截断**：工具结果超过4MB时自动截断为摘要，避免超出通义千问的API限制
- **对话持久化**：流式输出完成后，将完整文本保存到 `ConversationHistoryService`

### 6.3 修改的文件

#### `AIQueryController.java` — 新增流式查询端点

```java
@PostMapping(value = "/query/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public Flux<ServerSentEvent<String>> processUserQueryStream(
    @RequestBody AIQueryRequest request, HttpSession httpSession) {
    // 复用 buildEnhancedIntelligentContext() 构建上下文
    // 委托 aiStreamService.processStreamQuery() 处理
}
```
...
前端通过 `EventSource` 或 `fetch` API 以 SSE 协议连接 `/api/ai/query/stream`，逐条接收事件并实时渲染到聊天界面。
...
| 配置项 | 所属节 | 原始值 | 调整后 | 原因 |
|:---|:---|:---|:---|:---|
| `sample-window-minutes` | eeg.realtime-analysis | 2 | 1 | 缩小窗口关注更近期数据 |
| `min-samples` | eeg.realtime-analysis | 10 | 2 | 降低门槛适配数据量少的场景 |
| `analysis-interval-seconds` | eeg.realtime-analysis | 30 | 7 | 缩短间隔提升实时性 |
| `enable-auto-start` | eeg.realtime-analysis | true | false | 关闭自动启动，改为用户手动触发 |

### 7.2 变更涉及的代码文件清单

| 变更类型 | 文件 | 说明 |
|:---|:---|:---|
| 新增 | `AIStreamService.java` | SSE流式AI输出的完整实现 |
| 修改 | `AIQueryController.java` | 新增 `POST /api/ai/query/stream` 端点和 `AIStreamService` 依赖 |
| 修改 | `application.yml` | 调整实时分析参数、关闭自动启动 |
